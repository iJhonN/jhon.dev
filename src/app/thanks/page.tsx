...
export default function Thanks() {
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-slate-300 font-sans relative overflow-x-hidden">
      ...
      <div className="max-w-4xl mx-auto px-6 py-12 z-10 relative text-center">
        ...
        <p className="text-slate-400 max-w-xl mx-auto mb-16 leading-relaxed">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {supporters.map((name) => (
            <div 
              key={name}
              className="group relative p-6 rounded-2xl border border-white/5 bg-white/[0.02] backdrop-blur-sm transition-all hover:border-blue-600/50 hover:bg-white/[0.05]"
            >
              <div className="absolute inset-0 bg-blue-600/5 opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl" />
              <p className="relative z-10 text-white font-bold tracking-tight group-hover:text-blue-400 transition-colors">
                {name}
              </p>
            </div>
          ))}
